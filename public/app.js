...
// Tell the server an alarm was acknowledged (stops re-nudges).
...
function renderToday(sched) {
  ...
  if (upcoming.length) { $('#nextTime').textContent = upcoming[0]; $('#nextSub').textContent = '오늘 남은 알림 ' + upcoming.length + '회'; }
  else { $('#nextTime').textContent = '—'; $('#nextSub').textContent = times.length ? '오늘 알림이 모두 지나갔어요' : '오늘 예정된 알림이 없어요'; }
  const chips = $('#todayChips');
  chips.innerHTML = '';
  if (!times.length) chips.innerHTML = '<span class="hint">제외 시간대 때문에 오늘은 울릴 시간이 없어요.</span>';
  else times.forEach((t) => {
    const el = document.createElement('span');
    el.className = 'chip' + ((fired.includes(t) || t <= now) ? ' done' : '');
    el.textContent = t; chips.appendChild(el);
  });
...
$('#exclList').addEventListener('click', (e) => {
  const node = e.target.closest('.excl'); if (!node) return;
  const ex = findExcl(node.dataset.id); if (!ex) return;
  if (e.target.dataset.act === 'del') { config.exclusions = config.exclusions.filter((x) => x.id !== ex.id); renderExclusions(); return; }
...
$('#saveBtn').addEventListener('click', async () => {
  ...
  try {
    if (await isSubscribed()) {
      ...
      lastSchedule = r.schedule; renderToday(r.schedule);
